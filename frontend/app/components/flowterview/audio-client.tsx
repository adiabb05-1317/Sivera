 ...
 */
export function AudioClient({ onClearTranscripts, roomUrl }: AudioClientProps) {
  ...
  const { isSpeakerOn, isMicMuted, setIsMicMuted } = usePathStore()
  ...
  const checkPermissionAndConnect = async () => {
    try {
      ...
      // 1. Get a meeting room URL from backend
      ...
      if (!finalRoomUrl) {
        try {
          ...
          if (!response.ok) throw new Error("Failed to get room URL from backend")
          const data = await response.json()
          finalRoomUrl = data.room_url || data.url || data.roomUrl
          if (!finalRoomUrl) throw new Error("Room URL missing in backend response")
        } catch (err) {
          showToast("Could not connect to backend: " + (err instanceof Error ? err.message : err), "error")
          setConnectionStatus("disconnected")
          return
        }
      }
      ...
      rtviClientRef.current = new RTVIClient({
        params: {
          baseUrl: process.env.NEXT_PUBLIC_PIPECAT_BASE_URL || "http://localhost:8000/api/v1",
        ...
        callbacks: {
          ...
          onBotConnected: () => {
            ...
            // setTimeout(setupMediaTracks, 1000);
          },
          onError: (err: any) => {
          ...
          onGenericMessage: (data: any) => {
            ...
            try {
              ...
              if (message && typeof message === "object") {
                // Handle sources data first as it contains the most reliable image data
                ...
                // Fallback to direct image properties if no sources
                if (message.image || message.imageFile || message.url) {
                  const rawUrl =
                    message.image || message.imageFile || message.url
                  console.log("Raw image URL found:", rawUrl)
                  const imageUrl = formatImageUrl(rawUrl)
                  console.log("Formatted image URL:", imageUrl)

                  return // Exit early since we found and processed the image
                }

                // Check for nested image data as last resort
                if (
                  message.data &&
                  (message.data.image ||
                  ...
                  const rawUrl =
                    ...
                    message.data.imageFile ||
                    message.data.url
                  console.log("Raw nested image URL found:", rawUrl)
                  const imageUrl = formatImageUrl(rawUrl)
                  console.log("Formatted nested image URL:", imageUrl)
                }
              }
            } catch (error) {
  ...
  useEffect(() => {
    if (roomUrl && permissionGranted && !rtviClientRef.current) {
      console.log("Auto-connecting to Daily room:", roomUrl)
      checkPermissionAndConnect()
    }
  }, [roomUrl, permissionGranted])
