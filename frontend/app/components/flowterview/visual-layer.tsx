...
const VisualLayer = ({ className }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    if (hasSourceChanged) {
      ...
      if (sources[0]?.metadata?.imageFile) {
        setCurrentImageSrc(sources[0]?.metadata?.imageFile)
      } else if (sources[0]?.metadata?.source_demo_url) {
        setCurrentImageSrc(sources[0]?.metadata?.source_demo_url)
      }
  ...
  const handleImageLoad = () => {
    ...
    if (
      displayedSource?.metadata?.imageFile === currentImageSrc ||
      displayedSource?.metadata?.source_demo_url === currentImageSrc ||
      ...
      {displayedSource && (
        <>
          {displayedSource.metadata.type === "image" && (
            ...
            >
              <Image
                key={displayedSource.metadata.imageFile}
                src={displayedSource.metadata.imageFile!}
                ...
                onError={(e) => {
                  console.error(
                    ...
                    displayedSource.metadata.imageFile,
                    e
                  )
          ...
          {displayedSource.metadata.type === "layerpath/demo" && (
            <div
              className={cn(
                "w-full h-full transition-opacity duration-500",
                imageLoaded && !isLoading ? "opacity-100" : "opacity-0"
            ...
            >
              <iframe
                key={displayedSource.metadata.source_demo_url}
                style={{
                  position: "absolute",
                  top: "0",
                  left: "0",
                ...
                src={displayedSource.metadata.source_demo_url}
                onLoad={handleImageLoad}
                onError={(e) => {
                  console.error(
                    "iframe failed to load:",
                    displayedSource.metadata.source_demo_url,
                    e
                  )
                  // Force display even on error
                  setImageLoaded(true)
