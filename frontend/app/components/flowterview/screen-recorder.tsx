...
export function ScreenRecorder({
      ...
      try {
        ...
        if (videoTrack) {
          ...
          videoTrack.addEventListener("ended", async () => {
            ...
            toast.error(
              "Screen sharing ended unexpectedly. The interview has been terminated.",
              {
                description:
                  "Please refresh the page to start a new interview.",
              }
            );
          });
  ...
  useEffect(() => {
    ...
    if (callStatus === "left" && isRecording && hasStarted) {
      // Ensure we don't stop if already stopping
      console.log(
        "✅ LEGITIMATE call end detected - stopping recording after delay"
      );
