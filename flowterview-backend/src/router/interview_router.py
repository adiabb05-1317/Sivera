...
async def send_verification_email(email: str, name: str, job: str, token: str) -> None:
    ...
    try:
        ...
        verify_url = (
            f"{os.getenv('FRONTEND_URL', 'http://localhost:3001')}/interview?token={encoded_token}"
        )
...
async def send_invite(
    ...
    try:
        ...
        if user:
            ...
            interview_url = (
                f"{os.getenv('FRONTEND_URL', 'http://localhost:3001')}/interview/{interview_id}"
            )
        ...
        if "foreign key constraint" in str(e).lower():
            ...
            "smtp" in str(e).lower() or "email" in str(e).lower() or "aiosmtplib" in str(e).lower()
        ):
            raise HTTPException(
                status_code=500,
